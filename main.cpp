...
static void poll_ble_serial()
{
    while (bt.readable()) {
        ...
        if (ble_line_len == 0 && (c == '1' || c == '2' || c == '3'|| c == '4')) {
            if (c == '1') {
                ...
                buggy.drive(612,583);
                char vel_msg[48];
                snprintf(vel_msg, sizeof(vel_msg), "VEL L=%.4f,R=%.4f", leftEncoder.getVelocity(), rightEncoder.getVelocity());
    ...
    while (true) {
        ...
        if (lineTimer >= LINE_DT) {
            ...
            if (max_val < 0.243f) {
                ...
                filtered_position = 0.6f * raw_position_last + 0.4f * filtered_position;

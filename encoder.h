...
class WheelEncoder {
    private:
        ...
        float rpm;
    public:
        WheelEncoder(PinName A, PinName B, PinName index, float diam, float gear_ratio, int cpr) : enc(A, B, index, cpr, QEI::X2_ENCODING), t() {
            finalCPR = cpr * 2.0f; //effective cpr for x2 encoding mode
            functionVal = (M_PI * diam) / (finalCPR * gear_ratio); //calculates the value for function f(x,y,z)
            diameter = diam ;
